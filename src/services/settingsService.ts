...
export const settingsService = {
  ...
  async load(): Promise<AppSettings> {
    if (shouldUseDemoMode()) {
      return demoRepository.settings.load();
    }
  ...
  async set<K extends keyof AppSettings>(key: K, value: AppSettings[K]): Promise<void> {
    if (shouldUseDemoMode()) {
      return demoRepository.settings.set(key, value);
    }
  ...
  async setMany(settings: Partial<AppSettings>): Promise<void> {
    if (shouldUseDemoMode()) {
      return demoRepository.settings.setMany(settings);
    }
  ...
  async get<K extends keyof AppSettings>(key: K): Promise<AppSettings[K]> {
    if (shouldUseDemoMode()) {
      return demoRepository.settings.get(key);
    }
  ...
  async reset(): Promise<void> {
    if (shouldUseDemoMode()) {
      return demoRepository.settings.reset();
    }
